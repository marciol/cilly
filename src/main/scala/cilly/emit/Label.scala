...
object Label {
    ...
    final class SpecialLabel(kind: Label.Kind) extends Label {
        ...
        def setStacksize(stacksize: Int): Unit = { throw new RuntimeException("" + kind) }
        def incStacksize(): Unit = { throw new RuntimeException("" + kind) }
        def getKind(): Kind = kind
        def mergeWith(that: Label): Unit = { throw new RuntimeException("" + kind) }
