...
public abstract class Table {
  ...
  public static Table newTable(PEFile file, int id, int rows) {
    ...
    switch (id) {
    case ModuleDef.ID:
      table = new ModuleDef(file, rows);
      break;
    case TypeRef.ID:
      table = new TypeRef(file, rows);
      break;
    case TypeDef.ID:
      table = new TypeDef(file, rows);
      break;
    case FieldTrans.ID:
      table = new FieldTrans(file, rows);
      break;
    case FieldDef.ID:
      table = new FieldDef(file, rows);
      break;
    case MethodTrans.ID:
      table = new MethodTrans(file, rows);
      break;
    case MethodDef.ID:
      table = new MethodDef(file, rows);
      break;
    case ParamDef.ID:
      table = new ParamDef(file, rows);
      break;
    case InterfaceImpl.ID:
      table = new InterfaceImpl(file, rows);
      break;
    case MemberRef.ID:
      table = new MemberRef(file, rows);
      break;
    case Constant.ID:
      table = new Constant(file, rows);
      break;
    case CustomAttribute.ID:
      table = new CustomAttribute(file, rows);
      break;
    case FieldMarshal.ID:
      table = new FieldMarshal(file, rows);
      break;
    case DeclSecurity.ID:
      table = new DeclSecurity(file, rows);
      break;
    case ClassLayout.ID:
      table = new ClassLayout(file, rows);
      break;
    case FieldLayout.ID:
      table = new FieldLayout(file, rows);
      break;
    case StandAloneSig.ID:
      table = new StandAloneSig(file, rows);
      break;
    case EventMap.ID:
      table = new EventMap(file, rows);
      break;
    case EventDef.ID:
      table = new EventDef(file, rows);
      break;
    case PropertyMap.ID:
      table = new PropertyMap(file, rows);
      break;
    case PropertyDef.ID:
      table = new PropertyDef(file, rows);
      break;
    case MethodSemantics.ID:
      table = new MethodSemantics(file, rows);
      break;
    case MethodImpl.ID:
      table = new MethodImpl(file, rows);
      break;
    case ModuleRef.ID:
      table = new ModuleRef(file, rows);
      break;
    case TypeSpec.ID:
      table = new TypeSpec(file, rows);
      break;
    case ImplMap.ID:
      table = new ImplMap(file, rows);
      break;
    case FieldRVA.ID:
      table = new FieldRVA(file, rows);
      break;
    case AssemblyDef.ID:
      table = new AssemblyDef(file, rows);
      break;
    case AssemblyProcessor.ID:
      table = new AssemblyProcessor(file, rows);
      break;
    case AssemblyOS.ID:
      table = new AssemblyOS(file, rows);
      break;
    case AssemblyRef.ID:
      table = new AssemblyRef(file, rows);
      break;
    case AssemblyRefProcessor.ID:
      table = new AssemblyRefProcessor(file, rows);
      break;
    case AssemblyRefOS.ID:
      table = new AssemblyRefOS(file, rows);
      break;
    case FileDef.ID:
      table = new FileDef(file, rows);
      break;
    case ExportedType.ID:
      table = new ExportedType(file, rows);
      break;
    case ManifestResource.ID:
      table = new ManifestResource(file, rows);
      break;
    case NestedClass.ID:
      table = new NestedClass(file, rows);
      break;
  ...
  public final void readRow(int row) {
    ...
    populateFields();
    int rowSizeRead = buffer.position() - lastSeek;
  ...
  public static final class MethodSemantics extends Table {
    ...
    public boolean isGetter() {
      return (Semantics & Getter) != 0;
    }

    public boolean isSetter() {
      return (Semantics & Setter) != 0;
    }

    public boolean isOther() {
      return (Semantics & Other) != 0;
    }

    public boolean isAddOn() {
      return (Semantics & AddOn) != 0;
    }

    public boolean isRemoveOn() {
      return (Semantics & RemoveOn) != 0;
    }

    public boolean isFire() {
      return (Semantics & Fire) != 0;
    }
  ...
  public static final class GenericParam extends Table {
    ...
    private final java.util.Map<Integer, Set<Integer>> GenericParamIdxesForMethodDefIdx =
        new java.util.HashMap<Integer, Set<Integer>>();
    private final java.util.Map<Integer, Set<Integer>> GenericParamIdxesForTypeDefIdx =
        new java.util.HashMap<Integer, Set<Integer>>();

    private void addToMap(int key, int value, java.util.Map<Integer, Set<Integer>> IdxesForIdx) {
      java.util.Set<Integer> bucket = IdxesForIdx.get(Integer.valueOf(key));
      if (bucket == null) {
        bucket = new java.util.HashSet<Integer>();
    ...
    public int[] getTVarIdxes(int TypeDefIdx) {
      ...
      Set<Integer> bucket = GenericParamIdxesForTypeDefIdx.get(Integer.valueOf(TypeDefIdx));
      if (bucket == null) {
    ...
    public int[] getMVarIdxes(int MethodDefIdx) {
      ...
      Set<Integer> bucket = GenericParamIdxesForMethodDefIdx.get(Integer.valueOf(MethodDefIdx));
      if (bucket == null) {
        bucket = Collections.emptySet();
      }
      int[] res = new int[bucket.size()];
  ...
  public static final class GenericParamConstraint extends Table {
    ...
    public int[] getTypeDefOrRefIdxes(int genParamIdx) {
      ...
      Set<Integer> bucket = TypeDefOrRefIdxesForGenParamIdx.get(Integer.valueOf(genParamIdx));
      if (bucket == null) {
        bucket = Collections.emptySet();
      }
      int[] res = new int[bucket.size()];
    ...
    private final java.util.Map<Integer, Set<Integer>> TypeDefOrRefIdxesForGenParamIdx =
        new java.util.HashMap<Integer, Set<Integer>>();

    private void addToMap(int key, int value) {
      java.util.Set<Integer> bucket = TypeDefOrRefIdxesForGenParamIdx.get(Integer.valueOf(key));
      if (bucket == null) {
        bucket = new java.util.HashSet<Integer>();
