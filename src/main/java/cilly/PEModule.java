...
final class PEModule extends Module {
    ...
    protected void loadTypes() {
        ...
        for (int row = 2; row <= nbTypes; row++) {
            ...
            typesMap.put(name, new Integer(row)); // FIXME
        }
    ...
    /**
     * Return the type referenced by the given row in the TypeRef table only if
     * it resides in the given assembly. <i>Used by initCustomAttributes to
     * avoid unnecessary loading of referenced assemblies.</i>
     */
    Type getTypeRef(int row, Assembly inAssembly) {
        ...
        case AssemblyRef.ID:
            ...
            if (type == null) {
                // HACK: the IKVM.OpenJDK.Core assembly is compiled against
                // mscorlib.dll v2.0
                // The MSIL library cannot parse the v2.0 mscorlib because of
                // generics, so we
                // use the v1.0
                // However, the java.io.FileDescriptor.FlushFileBuffers method
                // uses a type
                // Microsoft.Win32.SafeHandles.SafeFileHandle, which only exists
                // in mscorlib
                // v2.0
                // For now, jsut return Object (fine as long as we don't use
                // that method).
    ...
    public Type getTypeDefOrRef(int index) {
        ...
        case Table.TypeSpec.ID:
            ...
            // GENERICINST (CLASS | VALUETYPE) TypeDefOrRefEncodred GenArgCount
            // Type*
            ...
            /*
             * Miguel says: Actually the following grammar rule production is
             * not among those for a TypeSpecBlob but I've found it in
             * assemblies compiled from C# 3.0. See also duplicate code in
             * PEFile.java
             */
            ...
            case Signature.ELEMENT_TYPE_SZARRAY: // Single-dim array with 0
                                                 // lower bound.
                sig.skipCustomMods();
            ...
            default:
                // TODO remaining grammar productions in 23.2.14 are for PTR and
                // FNPTR only
    ...
    private byte[] compressUInt(int u) {
         ...
         * http://www.cnblogs.com/AndersLiu/archive/2010/02
         * /09/en-compressed-integer-in-metadata.html
         */
        if (u <= 127 && 0 <= u) {
