...
public abstract class Type extends MemberInfo {
    ...
    private final java.util.List<GenericParamAndConstraints> tVars = new java.util.LinkedList<GenericParamAndConstraints>();
    ...
    public GenericParamAndConstraints[] getSortedTVars() {
        if (sortedTVars == null) {
            ...
            for (int i = 0; i < sortedTVars.length; i++) {
                Iterator<GenericParamAndConstraints> iter = tVars.iterator();
                while (iter.hasNext()) {
    ...
    public final int memberType() {
        return declaringType == null ? MemberTypes.TypeInfo
    ...
    protected Type(Module module, int attr, String fullName, Type baseType,
            Type[] interfaces, Type declType, int auxAttr, Type elemType) {
        super(fullName.lastIndexOf(Delimiter) < 0 ? fullName : fullName
                .substring(fullName.lastIndexOf(Delimiter) + 1,
                        fullName.length()), declType);

        this.Module = module; // null only for TMVarUsage and for PrimitiveType
        this.Attributes = attr;
        this.baseType = baseType;
        if (declType == null) {
            this.fullName = fullName;
            int i = fullName.lastIndexOf(Delimiter);
            this.namespace = (i < 0) ? "" : fullName.substring(0, i);
        } else {
            this.fullName = declType.fullName + "+" + fullName;
            this.namespace = declType.namespace;
        }

        this.interfaces = interfaces;
        this.elemType = elemType;
        this.auxAttr = auxAttr;
        this.name = fullName.lastIndexOf(Delimiter) < 0 ? fullName : fullName
                .substring(fullName.lastIndexOf(Delimiter) + 1,
                        fullName.length());
    ...
    public boolean canBeTakenAddressOf() {
        /*
         * TODO should be overridden in TMVarUsage, but there's currently no way
         * to bind a TMVarUsage to its GenericParamAndConstraints definition.
         * Why? Because of the way the msil library is organized (e.g.,
         * mkArray() returns the same !0[] representation for all !0[] usages,
         * irrespective of the scope of the !0 type-param) This in turn is so
         * because without generics there's no harm in using a type-def instance
         * where a type-ref should go (e.g., the ParameterType of a
         * ParameterInfo nowadays may point to a PEType). The net effect is that
         * this method (CanBeTakenAddressOf) is conservative, it will answer
         * "no" for example for !0 where !0 refers to a type-param with the
         * isValuetype constraint set. The whole thing is ok at this point in
         * time, where generics are not supported at the backend.
         */
    ...
    protected Type(Module module, int attr, String fullName, Type baseType,
            Type[] interfaces, Type declType, int auxAttr) {
        this(module, attr, fullName, baseType, interfaces, declType, auxAttr,
                null);
    }

    protected Type(Type instantiatedType) {
    ...
    public static final class TMVarUsage extends Type {
        ...
        public TMVarUsage(int number, boolean isTVar) {
            super(null, 0, ((isTVar ? "!" : "!!") + number), null, null, null,
                    AuxAttr.None, null);
            this.number = number;
            this.isTVar = isTVar;
        }
    ...
    public static Type mkArray(Type elemType, int rank) {
        ...
        array = new PrimitiveType(elemType.Module, elemType.Attributes
                ...
                elemType.fullName + arrSig, ARRAY(), EmptyTypes, null,
    ...
    public static Type mkPtr(Type elemType) {
        ...
        type = new PrimitiveType(elemType.Module, elemType.Attributes, name,
                null, EmptyTypes, null, AuxAttr.Pointer, elemType);
        return addType(type);
    }
    ...
    public static Type mkByRef(Type elemType) {
        ...
        if (type != null)
            return type;
        type = new PrimitiveType(elemType.Module, elemType.Attributes, name,
                null, EmptyTypes, null, AuxAttr.ByRef, elemType);
        return addType(type);
    }
    ...
    public static Type getType(String fullName) {
        ...
        // check if it's an array type; TODO: make array type handling more
        // robust
            ...
            if (j > i && j == (fullName.length() - 1)) {
                ...
                for (int k = i + 1; k < j; k++) {
                    if (fullName.charAt(k) != ',')
                        throw new RuntimeException("Malformed type name: "
                                + fullName);
                }
        ...
        // System.out.println("Looking for type: " + fullName + " (" +
        // fullName.length() + ")");
        // try in the assemblies
        Iterator<Assembly> assems = cilly.Assembly.assemblies.values()
                .iterator();
        while (type == null && assems.hasNext()) {
            Assembly assem = (assems.next());
    ...
    public FieldInfo[] getFields(int bindingFlags) {
        initFields();
        final FieldInfo[] fields = getAllFields((bindingFlags & BindingFlags.DeclaredOnly) != 0);
        ...
        final boolean getNonPublic = (bindingFlags & BindingFlags.NonPublic) != 0;
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (accessible
                    ...
                    && ((field.declaringType == this) || ((field.declaringType != this) && !field
                            .isPrivate()))
                    && ((getInstance && !field.isStatic()) || ((getStatic && field
                            .isStatic()) && (field.declaringType == this || (bindingFlags & BindingFlags.FlattenHierarchy) != 0))))
                fields[cnt++] = field;
        }
    ...
    protected FieldInfo[] getAllFields(boolean declaredOnly) {
        initFields();
        FieldInfo[] inherited = BaseType() == null || declaredOnly ? FieldInfo.EMPTY_ARRAY
                : BaseType().getAllFields(declaredOnly);
        FieldInfo[] allFields = new FieldInfo[inherited.length
                + this.fields.length];
        System.arraycopy(inherited, 0, allFields, 0, inherited.length);
        System.arraycopy(this.fields, 0, allFields, inherited.length,
                this.fields.length);
        return allFields;
    }
    ...
    /**
     */
    protected static MethodInfo findMethod(MethodInfo[] methods, String name,
            Type[] paramTypes, Type retType) {
        for (int i = 0; i < methods.length; i++)
    ...
    protected static boolean equalParameters(ParameterInfo[] params,
        ...
        for (int i = 0; i < params.length; i++) {
            // System.out.println(params[i].ParameterType + " == " +
            // paramTypes[i]
    ...
    public MethodInfo[] getMethods(int bindingFlags) {
        initMethods();
        final MethodInfo[] methods = getAllMethods((bindingFlags & BindingFlags.DeclaredOnly) != 0);
        // System.out.println("" + this + ".GetMethods(int) -> " +
        // methods.length);
        final boolean getInstance = (bindingFlags & BindingFlags.Instance) != 0;
        final boolean getStatic = (bindingFlags & BindingFlags.Static) != 0;
        final boolean getPublic = (bindingFlags & BindingFlags.Public) != 0;
    ...
    protected MethodInfo[] getAllMethods(boolean declaredOnly) {
        initMethods();
        MethodInfo[] inherited = BaseType() == null || declaredOnly ? MethodInfo.EMPTY_ARRAY
                : BaseType().getAllMethods(declaredOnly);
        MethodInfo[] allMethods = new MethodInfo[inherited.length
                + this.methods.length];
        System.arraycopy(inherited, 0, allMethods, 0, inherited.length);
        System.arraycopy(this.methods, 0, allMethods, inherited.length,
                this.methods.length);
        return allMethods;
    }
    ...
    public Type getInterface(String name, boolean ignoreCase) {
        ...
        for (int i = 0; i < interfaces.length; i++) {
            ...
            if (ignoreCase) {
                if (name.equalsIgnoreCase(iface.name))
                    return iface;
                if (name.equalsIgnoreCase(iface.fullName))
                    return iface;
            } else {
                if (name.equals(iface.name))
                    return iface;
                if (name.equals(iface.fullName))
                    return iface;
            }
        }
        return BaseType() == null ? null : BaseType().getInterface(name,
                ignoreCase);
    }
    ...
    public boolean isSubtypeOf(Type that) {
        if (this == that || BaseType() == that || that == OBJECT())
            return true;
        initInterfaces();
        for (int i = 0; i < interfaces.length; i++)
    ...
    private static String formatType(Type t) {
        ...
        return "[" + t.assembly().getName() + "]" + t + "(" + cname + "#"
    ...
    public MemberInfo[] getMember(String name) {
        aggregateMembers();
        List<MemberInfo> l = new ArrayList<MemberInfo>();
    ...
    protected void aggregateMembers() {
        ...
        List<MemberInfo> l = new ArrayList<MemberInfo>(Arrays.asList(fields));
        ...
        members = l.toArray(MemberInfo.EMPTY_ARRAY());
    }
     ...
     */
    public Type[] getDeclaredNestedTypes() {
        initNestedTypes();
