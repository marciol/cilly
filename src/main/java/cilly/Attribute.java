...
public class Attribute {
    ...
    public NamedArgument[] getNamedArguments() {
        NamedArgument[] nargs = namedArgs.values().toArray(NamedArgument.EMPTY);
        return nargs;
    }
    ...
    private static final Map<Type, Integer> type2id = new HashMap<Type, Integer>();
    private static final Map<Integer, Type> id2type = new HashMap<Integer, Type>();
    static {
    ...
    private void parseBlob0() {
        ...
        namedArgs = new LinkedHashMap<String, NamedArgument>();
        for (int i = 0; i < ncount; i++) {
            int designator = buf.get(); // designator one of 0x53 (FIELD) or
                                        // 0x54 (PROPERTY)
            assert designator == Signature.X_ELEMENT_KIND_FIELD
                    || designator == Signature.X_ELEMENT_KIND_PROPERTY : "0x"
                    + PEFile.byte2hex(designator);
            ...
            NamedArgument narg = new NamedArgument(designator, name, type,
                    value);
    ...
    /*
     * indicates whether the "simple" case (the other is "enum") of the first
     * row in the Elem production should be taken.
     */
    ...
    /*
     * indicates whether the second row in the Elem production should be taken
     * (and more specifically, "string" case within that row).
     */
    ...
    /*
     * indicates whether the second row in the Elem production should be taken
     * (and more specifically, "type" case within that row).
     */
    ...
    /*
     * indicates whether the third row in the Elem production should be taken
     * (and more specifically, "boxed" case within that row).
     */
    ...
    private Object parseElem(Type type) {
        ...
        // boxed valuetype, please notice that a "simple" boxed valuetype is
        // preceded by 0x51
        if (isSystemObject(type)) {
            Type boxedT = parse0x51();
            if (boxedT.isEnum()) {
                return new BoxedArgument(boxedT,
                        parseVal(getTypeId(boxedT.getUnderlyingType())));
            } else {
                return new BoxedArgument(boxedT, parseVal(getTypeId(boxedT))); // TODO
                                                                               // dead
                                                                               // code?
            }
        } else {
    ...
    /*
     * this does not parse an Elem, but a made-up production (Element). Don't
     * read too much into this method name!
     */
    private Object parseVal(int id) {
        ...
        case Signature.ELEMENT_TYPE_STRING:
            return parseString();
        default:
            throw new RuntimeException("Shouldn't have called parseVal with: "
                    + id);
        }
    }
    ...
    private Type parseType() { // FieldOrPropType, Sec. 23.3 in Partition II of
                               // CLR Spec.
        int id = buf.get();
        switch (id) {
    ...
    private Type parseFieldOrPropTypeInNamedArg() { // FieldOrPropType, Sec.
                                                    // 23.3 in Partition II of
                                                    // CLR Spec.
        int id = buf.get();
        switch (id) {
        case 0x51:
            return (id2type.get(new Integer(Signature.ELEMENT_TYPE_OBJECT)));
    ...
    private Type getTypeFromSerString() {
        ...
        /*
         * fully qualified assembly name follows. Just strip it on the
         * assumption that the assembly is referenced in the externs and the
         * type will be found.
         */
    ...
    public int parseLength() {
        ...
        // most significant bits should be 0; if all are 1 the length is
        // invalid.
        if ((length & 0xe0) == 0xe0)
            return -1;
        if ((length & 0x80) != 0) {
            ...
            if ((length & 0x4000) != 0)
                length = ((length & 0x3fff) << 16) | (getByte() << 8)
                        | getByte();
        }
        return length;
    }
    ...
    public static class NamedArgument {
        ...
        public NamedArgument(int designator, String name, Type type,
                Object value) {
